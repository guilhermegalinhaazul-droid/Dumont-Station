...
public sealed partial class GuideMicrowaveEmbed : PanelContainer, IDocumentTag, ISearchableControl
{
    ...
    public GuideMicrowaveEmbed(DeepFryerRecipePrototype recipe) : this()
    {
        GenerateControl(recipe);
    }
    ...
    private void GenerateHeader(DeepFryerRecipePrototype recipe)
    {
        var entity = _prototype.Index<EntityPrototype>(recipe.Result);
    ...
    private void GenerateSolidIngredients(DeepFryerRecipePrototype recipe)
    {
        foreach (var (product, amount) in recipe.Ingredients.OrderByDescending(p => p.Value))
        {
            var ingredient = _prototype.Index<EntityPrototype>(product);
            ...
            solidNameMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-solid-name-display",
            ...
            quantityMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-solid-quantity-display",
                    ("amount", amount)));
    ...
    private void GenerateCookTime(DeepFryerRecipePrototype recipe)
    {
        AppendMachineTexture(
            "/Textures/_Trauma/Structures/Machines/Service/deep_fryer_icon.rsi",
            "icon");
        ...
        msg.AddMarkupOrThrow(
            Loc.GetString(
                ...
                ("processingTypes",
                    Loc.GetString("guidebook-food-processing-type-deep-fryer")),
                ("time", recipe.Time)));
    ...
    private void GenerateControl(DeepFryerRecipePrototype recipe)
    {
        GenerateHeader(recipe);
        GenerateIngredients(recipe);
        GenerateCookTime(recipe);
